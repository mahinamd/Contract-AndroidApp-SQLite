...
public class KeyValueDB extends SQLiteOpenHelper {
    ...
    public final String TABLE_KEY_VALUE = "contact";
    ...
    private void createKeyValueTable(SQLiteDatabase db) {
        try {
            db.execSQL("create table " + TABLE_KEY_VALUE + "(" + KEY + " TEXT, " + VALUE + " TEXT, " + IMAGE + " Text)");
        } catch (Exception e) {
        }
    }
    ...
    public Boolean insertKeyValue(String key, String value, String image) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public boolean updateValueByKey(String key, String value, String image) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(KEY, key);
        cv.put(VALUE, value);
        ...
        if (nr == 0) {
            insertKeyValue(key, value, image);
        }
        return true;
    }
